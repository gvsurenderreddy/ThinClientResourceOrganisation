...
{
    [TestClass]
    public class EmployeeForename_is_a_valid_persons_name : IsAPersonsNameSpecification<HRTestBootstrap, AddEmployeeRequestHandler_fixture>
    {
        public EmployeeForename_is_a_valid_persons_name()
            : base(
                    set_request_field: (f, value) => f.request.forename = value,
                    act: f => f.execute_command(),
                    error_was_identified: f => Assert.IsTrue(f.response.has_status<AddEmployeeServiceStatuses.EmployeeForenameHasInvalidCharacters>()),
