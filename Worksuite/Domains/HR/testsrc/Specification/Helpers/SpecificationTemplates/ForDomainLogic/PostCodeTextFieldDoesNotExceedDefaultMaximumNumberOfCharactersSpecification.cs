...
{
    public abstract class PostCodeTextFieldDoesNotExceedDefaultMaximumNumberOfCharactersSpecification<P, Q, F> 
         : TextFieldDoesNotExceedMaximumNumberOfCharactersSpecification<P, Q, F>                
        where Q : Response
        where F : IsAResponseCommandFixture<P, Q> {

        protected PostCodeTextFieldDoesNotExceedDefaultMaximumNumberOfCharactersSpecification 
            ( Action<P, string> set_field_by ) : base( set_field_by, ValidationConstraints.max_text_field_length_for_post_code  ) {}
