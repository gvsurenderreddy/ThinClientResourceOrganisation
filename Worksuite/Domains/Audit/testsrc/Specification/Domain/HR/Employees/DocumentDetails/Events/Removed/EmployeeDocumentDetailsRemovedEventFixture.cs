...
using WTS.WorkSuite.Audit.HR.Employees.Documents;
...
{
    public class EmployeeDocumentDetailsRemovedEventFixture
    {
        public EmployeeDocumentRemovedEvent create_event()
        {
            ...
            return new EmployeeDocumentRemovedEvent
            {
                employee_id = audit_trail.employee_id,
                employee_document_id = (new Random()).Next(),
                name = "An important document",
                description = "Some description about the important document"
            };
        }

        public Maybe<EmployeeDocumentDetailsAuditRecord> get_last_employee_document_details_audit_record_for(EmployeeDocumentIdentity the_document)
        {
            return this
                .get_employee_audit_trail_for(new EmployeeIdentity { employee_id = the_document.employee_id })
                .Match(
                    ...
                    has_value:
                        audit_trail =>
                        {
                            ...
                            return audit_record != null
                                            ? new Just<EmployeeDocumentDetailsAuditRecord>(audit_record) as Maybe<EmployeeDocumentDetailsAuditRecord>
                                            : new Nothing<EmployeeDocumentDetailsAuditRecord>()
                                            ;
                        },

                        nothing:
                        () => (new Nothing<EmployeeDocumentDetailsAuditRecord>() as Maybe<EmployeeDocumentDetailsAuditRecord>)
                );
        }
        ...
        public AddDocumentDetailsAuditRecordWhenDocumentIsRemoved event_handler { get; private set; }

        public FakeClock clock { get; private set; }

        public EmployeeDocumentDetailsRemovedEventFixture()
        {
            event_handler = DependencyResolver.resolve<AddDocumentDetailsAuditRecordWhenDocumentIsRemoved>();
