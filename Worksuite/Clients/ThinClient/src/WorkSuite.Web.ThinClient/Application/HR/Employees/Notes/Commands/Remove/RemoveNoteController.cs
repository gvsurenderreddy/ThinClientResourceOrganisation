using System.Web.Mvc;
using WTS.WorkSuite.HR.HR.Employees.Notes.New;
using WTS.WorkSuite.HR.HR.Employees.Notes.Remove;
using WTS.WorkSuite.Library.CodeStrutures.Behavioral;

namespace WTS.WorkSuite.Web.ThinClient.Application.HR.Employees.Notes.Commands.Remove
{
    public class RemoveNoteController : Controller
    {
          public ActionResult SubmitRequest ( NoteIdentity remove_request ) {
         
            var remove_response = remove.execute( remove_request );

            Response.StatusCode = remove_response.has_errors ? 400 : 200;

            return Json( remove_response, JsonRequestBehavior.AllowGet );
        }


          public RemoveNoteController(IRemove remove_command)
        {

            remove = Guard.IsNotNull( remove_command, "remove_command" );

        }

        private readonly IRemove remove;
    }
}