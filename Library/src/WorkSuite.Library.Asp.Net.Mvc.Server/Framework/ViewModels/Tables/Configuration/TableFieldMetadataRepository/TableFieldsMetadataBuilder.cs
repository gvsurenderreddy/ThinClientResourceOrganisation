...
{
    public class TableFieldsMetadataBuilder<S> : ITableFieldsMetadataBuilder<S>
    {
        public ITableFieldMetadataBuilder<S> for_field<P>(Expression<Func<S, P>> property_expression)
        {
            ...
            if (property_expression.Body.NodeType == ExpressionType.MemberAccess)
            {
                ...
                return new TableFieldMetadataBuilder<S>(m => repository.add_metadata(property_name, m));
            }
            throw new Exception("Could not identify property from the expression.");
        }


        public TableFieldsMetadataBuilder 
            ( TableFieldMetadataRepository<S> the_repository ) {
        ...
        private readonly TableFieldMetadataRepository<S> repository;
    }
}