...
namespace WTS.WorkSuite.PlannedSupply.Services.Domain.OperationsCalendar.ShiftCalendar.Fields.EndDate
{

    [TestClass]
    public class EndDateShouldBeMandatory : MandatoryDateFieldSpecification  < PublishShiftCalendarRequest
                                                                             , PublishShiftCalendarResponse
                                                                             , PublishDatesShiftCalendarFixture>
    {
        public EndDateShouldBeMandatory()
            : base((request, end_date_request) => request.end_date = end_date_request)
        {
        }
    }

    [TestClass]
    public class CheckErrorMessageForEndDateShouldBeMandatory : PlannedSupplySpecification
    {
        [TestMethod]
        public void report_error_message_when_end_date_is_null()
        {
            fixture.request.end_date = new DateRequest { year = null, month = null, day = null};
            fixture.execute_command();
            var response = fixture.response;
            response.messages.Should().Contain(m => m.field_name == "end_date" && m.message == ValidationMessages.error_210220151519);
            response.messages.Should().Contain(m => m.message == WarningMessages.warning_210220151514);
        }

        [TestMethod]
        public void show_red_border_around_fields_when_end_date_is_null()
        {
            fixture.request.end_date = new DateRequest { year = null, month = null, day = null };
            fixture.execute_command();
            var response = fixture.response;
